...
class ContentBlockUtil
{
    ...
    private static function getBy($where)
    {
        ...
        $m = $m->toArray();
        ModelUtil::decodeRecordJson($m, ['images', 'texts', 'textsMo', 'imagesMo']);
        if ($m['image']) {
            $m['image'] = AssetsUtil::fixFull($m['image']);
        }
        if ($m['images']) {
            $m['images'] = AssetsUtil::fixFull($m['images'], false);
            $m['imagesMo'] = AssetsUtil::fixFull($m['imagesMo'], false);
        }
        return $m;
    }
    ...
    private static function allBy($where, $limit)
    {
        ...
        ModelUtil::decodeRecordsJson($ms, ['images', 'texts', 'textsMo', 'imagesMo']);
        foreach ($ms as $mIndex => $m) {
            $ms[$mIndex]['images'] = AssetsUtil::fixFull($m['images'], false);
            $ms[$mIndex]['imagesMo'] = AssetsUtil::fixFull($m['imagesMo'], false);
        }
        return $ms;
    }
    ...
    public static function getModel($name, $model, $limit=0)
    {
        return self::allBy([
            'name' => $name,
            'model' => $model,
    ...
    public static function getCachedModel($name, $model, $limit = 0)
    {

        return CacheUtil::remember(self::CACHE_KEY_PREFIX . "Name:" . $name . $model,
            3600,
            function () use ($name, $model) {
                return self::getModel($name, $model);
            });
    }
