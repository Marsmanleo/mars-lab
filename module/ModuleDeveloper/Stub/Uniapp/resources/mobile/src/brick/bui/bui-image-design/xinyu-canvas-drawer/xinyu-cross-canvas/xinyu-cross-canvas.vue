	...
	export default {
		...
		methods: {
			...
			async callContextMethod(methodName, methodParams) {
				...
				} else if (methodName === "maskColor") {
                    const imageData = this.context.getImageData( 0, 0, this.canvas.width, this.canvas.height);
                    const fromColor = methodParams[0];
                    const toColor = methodParams[1];
                    let imageDataContent = imageData.data;
                    for (let i = 0; i < imageDataContent.length; i += 4) {
                        if(imageDataContent[i] === fromColor.r && imageDataContent[i + 1] === fromColor.g && imageDataContent[i + 2] === fromColor.b){
                            imageData.data[i] = toColor.r;
                            imageData.data[i + 1] = toColor.g;
                            imageData.data[i + 2] = toColor.b;
                        }
                    }
                    return await this.context.putImageData(imageData, 0, 0);
                }
				return this.context[methodName].apply(this.context, methodParams);
				// #endif
				// #ifndef MP
                // console.log('callContextMethod', methodName, methodParams)
	...
	export default {
		...
		methods: {
			...
			async callContextMethodRenderjs(prop) {
				...
				if (prop.methodName == "drawImage") {
				...
				} else if(prop.methodName == 'maskColor') {
				    const imageData = this.contextRenderJS.getImageData( 0, 0, this.canvasRenderJS.width, this.canvasRenderJS.height);
				    const fromColor = prop.methodParams[0];
				    const toColor = prop.methodParams[1];
				    let imageDataContent = imageData.data;
                    for (let i = 0; i < imageDataContent.length; i += 4) {
                        if(imageDataContent[i] === fromColor.r && imageDataContent[i + 1] === fromColor.g && imageDataContent[i + 2] === fromColor.b){
                            imageData.data[i] = toColor.r;
                    ...
                    this.contextRenderJS.putImageData(imageData, 0, 0);
				} else
