                ...
                <view v-if="item.style==='coverTitle'&&item.cover">
                    <view
                        ...
                        <view class="tw-w-8 tw-text-right tw-flex tw-flex-col tw-justify-end"
                            ...
                            <text class="iconfont icon-heart ub-text-primary"></text>
                        </view>
                    </view>
                </view>
                <view v-else>
...
export default {
    ...
    methods: {
        init() {
            ...
            this.$api.post('member_like/info', {
                biz: this.biz
            }, res => {
        ...
        doList(page) {
            this.doListProcess('member_like/paginate', page, {
                biz: this.biz
            })
        },
        doCancel(biz, bizId) {
            this.$api.post('member_like/cancel', {
                biz,
                bizId
            }, res => {
                this.$dialog.tipSuccess('取消喜欢成功')
                this.doList()
            })
        }
    }
