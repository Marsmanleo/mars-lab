<template>
    <view @click="doSubmit" class="tw-h-10 tw-w-10 tw-leading-6 tw-overflow-hidden tw-text-center tw-relative">
        <view v-if="value" class="iconfont icon-heart ub-text-primary" style="font-size:1rem;margin-top:-0.2rem;">
        </view>
        <view v-if="!value" class="iconfont icon-heart-alt ub-text-muted" style="font-size:1rem;margin-top:-0.2rem;">
        </view>
    ...
    methods: {
        doSubmit() {
            this.requireUserLogin(() => {
                this.$api.post(`member_like/submit/${this.biz}`, {
                    biz: this.biz,
                    bizId: this.bizId
                }, res => {
