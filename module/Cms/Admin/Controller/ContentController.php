                ...
                function (Form $form) use ($catOptions) {
                    ...
                    $form->layoutPanel('基本信息', function (Form $form) use ($catOptions) {
                        ...
                        if (in_array($this->model['mode'], [CmsMode::LIST_DETAIL, CmsMode::PAGE])) {
                            ...
                            $form->image('cover', '封面');
                            $form->image('coverMo', '封面-手机');
                        }
                        if (in_array($this->model['name'], ['cases'])) {
                            $form->select('brand', '品牌')->options(['Space'=>'Space', 'Creative'=>'Creative',"Selection"=>"Selection","Tree"=>"Tree","Education"=>"Education","Business"=>"Business"]);
                            $form->image('brandImg', '品牌logo');
                        }
                        if (in_array($this->model['name'], ['brands'])) {
                            $form->textarea('desc', '描述');
                            $form->image('banner1', '展示页图1');
                            $form->image('banner2', '展示页图2');
                            $form->image('banner3', '展示页图3');
                        }
        ...
        if (Request::isPost()) {
            ...
            return $form->formRequest(function (Form $form) use ($record) {
                ...
                $recordValue = ArrayUtil::keepKeys($data, [
                    'catId', 'title', 'alias', 'title', 'summary', 'cover','coverMo', 'brand','brandImg', 'banner1', 'banner2', 'banner3', 'desc', 'postTime',
